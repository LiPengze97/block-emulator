...
type TestModule_avgTPS_Pyramid struct {
	epochID      int
...
func NewTestModule_avgTPS_Pyramid() *TestModule_avgTPS_Pyramid {
	return &TestModule_avgTPS_Pyramid{
		epochID:      -1,
...
func (tat *TestModule_avgTPS_Pyramid) OutputMetricName() string {
	return "Average_TPS"
}

// add the number of excuted txs, and change the time records
func (tat *TestModule_avgTPS_Pyramid) UpdateMeasureRecord(b *message.BlockInfoMsg) {
	...
	for _, tx := range b.PyramidTxs {
		tat.relayTx[string(tx.TxHash)] = true
	}
	tat.excutedTxNum[epochid] += float64(b.Relay1TxNum) / 2
	tat.excutedTxNum[epochid] += float64(b.ParamidTxNum) / 2
...
func (tat *TestModule_avgTPS_Pyramid) HandleExtraMessage([]byte) {}

// output the average TPS
func (tat *TestModule_avgTPS_Pyramid) OutputRecord() (perEpochTPS []float64, totalTPS float64) {
